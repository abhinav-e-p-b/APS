...
// Required to safely close the auth session on certain platforms
...
export default function LandingScreen({ navigation, onAdminLogin }) {
  ...
  async function handleGoogleLogin() {
    setLoading(true);
    try {
      // 1. Create a dynamic redirect URL (Handles both Expo Go and Standalone APK)
      const redirectUrl = Linking.createURL('/auth/callback');

      // 2. Fetch the Google OAuth URL, but stop Supabase from redirecting automatically
      ...
      // 3. Open the In-App Web Browser for the user to sign in
      if (data?.url) {
        const res = await WebBrowser.openAuthSessionAsync(data.url, redirectUrl);

        // 4. Intercept the success redirect and parse the session
        if (res.type === 'success') {
          const { url } = res;
          ...
          // 5. Establish the session manually using the intercepted tokens
  ...
  // ... keep handleAdminLogin and the rest of your UI return block exactly as it is ...
      ...
      >
        ...
        >
          ...
          <Text style={styles.heroTitle}>
            Skip the Queue,{'\n'}
            Park <Text style={{ color: COLORS.cyan }}>instantly.</Text>
          </Text>
